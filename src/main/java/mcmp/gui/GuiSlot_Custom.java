...
/**
 * GuiSlotクラスをカスタマイズしたクラスです。
 * @author tana
 */

@SideOnly(Side.CLIENT)
public abstract class GuiSlot_Custom
{
    ...
    public GuiSlot_Custom(Minecraft par1Minecraft, int par2, int par3, int par4, int par5, int par6)
    {
        ...
        for (int j1 = 0; j1 < i1; ++j1)
        {
            ...
            if (k1 <= this.bottom && k1 + l1 >= this.top)
            {
                if (this.showSelectionBox && this.isSelected(j1))
                {
                    ...
                    GL11.glDisable(GL11.GL_TEXTURE_2D);
                    tessellator.startDrawingQuads();
                    tessellator.setColorOpaque_I(0x00000000);
